...
`src/components/ui/RouteTransition.tsx` takes a **render prop**: `{(displayed) => <Routes location={displayed} />}` in `App.tsx`. `displayed` trails the live location by `EXIT_MS` during a transition — the swap is **deliberately timer-driven, not `AnimatePresence mode="wait"`**: React Router v7 wraps navigations in `startTransition`, and with React 18 that combo intermittently lost motion's exit-complete signal, stranding the URL on the old page (the "checkout click does nothing" bug). Don't "simplify" it back to AnimatePresence page swapping. Choreography: exit scale/fade → translucent liquid wave sweeps bottom-to-top (clip-path glass panels) → new page springs up. It skips the initial page load, is `pointer-events-none` throughout, and degrades to a plain crossfade under `prefers-reduced-motion`. In-app navigation must use `<Link>`/`<NavLink>`/`navigate()` — plain `<a href>` causes a full reload and bypasses the transition. Tuning dials (blur, tint alphas, `SWEEP_SECONDS`) live at the top of the file; to screenshot mid-sweep, temporarily raise `SWEEP_SECONDS`, then revert.
...
- **Schema tolerance has a deliberate carve-out here — don't "fix" it.** `placeOrder` never retries without the **address**: an order silently written without one is strictly worse than a failed order, because nobody can ship it and nobody finds out. It fails loudly instead. (It *does* retry once without `shipping_fee` — that column is display breakdown only, `total` already includes the fee, so the order stays correct and shippable on a DB missing the fee SQL.) The order **reads** (`fetchMyOrders`, `fetchAllOrders`) *are* tolerant of missing `shipping`/`shipping_fee` columns, because past orders without those lines beat no order history at all — and the retries are **ordered** `shipping_fee` first, since `/shipping/` also matches `shipping_fee`. Tolerance is about whether a default is meaningful, not a blanket rule.
- **Flat shipping fee**: `shop_settings` key `'shipping'` (`{"flatFee": n}`) → `fetchShippingConfig`/`upsertShippingConfig` (`catalog.ts`) → `registerShipping`/`getShippingFee` (`pricing.ts`, registry pattern) → checkout adds it to `total` and snapshots it in `orders.shipping_fee`. 0/missing = free shipping and hides every fee line. Never recompute an old order's fee from the live config — the snapshot is the truth.

### 3D bottle (`src/components/three/`)

React Three Fiber renders a procedural glass bottle. `LiquidLayers` pours three layers sized by live percentages, each colored by `noteColor` (its note's ingredient blend), merging to one color when "blended"; `BottleLabel` prints the name live. `Scene` drives the builder and is now the **only** WebGL entry point — the landing's 3D showcase was replaced by `NoteMarquee`, so no page but `/builder` creates a WebGL context. `Scene` is `React.lazy`-loaded in `ScentBuilder.tsx` so the three stack ships as its own async chunk; never add a static import of `components/three/*` reachable from `App.tsx` — it silently merges three.js back into the main bundle (check emitted chunk sizes, not just a green build).
...
### SEO & build-time prerender

`npm run build` also prerenders `/`, `/collection`, and `/builder` into `dist/<route>/index.html` via `vite-prerender-plugin` + `src/prerender.tsx` (`renderToString` in Node). Consequences: **public-route components must not touch `window`/`document` at render time** — guard with `typeof window === 'undefined'` (see `HeroSlider`); effects are fine, they never run under `renderToString`. `/builder` prerenders as a shell because the lazy `Scene` emits its Suspense fallback — another reason the three.js import must stay dynamic. No `vercel.json` change is needed: Vercel serves the prerendered files before the SPA rewrite, and unknown paths still fall back to `index.html`. Per-route title/meta/canonical/JSON-LD live in one place — `src/lib/seo.ts` (`ROUTE_META`, applied client-side by `applySeo` in `App.tsx`, baked in at build by `prerender.tsx`). `SITE_URL` there is a **placeholder domain**, duplicated in `public/robots.txt` and `public/sitemap.xml` — update all three when the real domain lands. `main.tsx` uses `createRoot().render()`, not hydration, so the static DOM is simply replaced at boot — don't switch to `hydrateRoot` without knowing that changes the mismatch rules.
